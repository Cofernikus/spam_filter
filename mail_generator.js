...
let words = [
    "fake", "legit", "coconut", "tarriff"
]

let names = [
    "ivo@hotmail.com", "tea@yahoo.co.uk", "marko@gmail.com", "filip@anonmail.co"
]

let recipients = [
    "fturkovic@tvz.hr", "tstapko@tvz.hr", "mhadinovic@tvz.hr", "fsolinko@tvz.hr"
]
...
function subject_generator(word_count){
    ...
    if(!(word_count<1)){
        for(let i = 0; i<word_count; i++){
            return_subject = return_subject.concat(' ', words[randInt(words.length,0)]);
        }
        return return_subject;
    }
    return return_subject = return_subject.concat('', words[randInt(words.length,0)].toString());
}
...
function post_body(){
    const data_to_push = axios.post('http://localhost:3000/api', generate_mail(), {
    headers: {
